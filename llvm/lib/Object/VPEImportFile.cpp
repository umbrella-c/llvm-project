//===- VPEImportFile.cpp - VPE short import file implementation ---------===//
//
...
static ImportNameType getNameType(StringRef Sym, StringRef ExtName,
  ...
  // details.
  if (ExtName.startswith("_") && ExtName.contains('@'))
    return IMPORT_NAME;
  if (Sym != ExtName)
...
ObjectFactory::createImportDescriptor(std::vector<uint8_t> &Buffer) {
  ...
  // COFF Header
  vpe_file_header Header{
      u16(Machine),
      u16(NumberOfSections),
      u32(0),
      u32(sizeof(Header) + (NumberOfSections * sizeof(vpe_section)) +
          // .idata$2
          sizeof(vpe_import_directory_table_entry) +
          NumberOfRelocations * sizeof(vpe_relocation) +
          // .idata$4
          (ImportName.size() + 1)),
      u32(NumberOfSymbols),
      u16(0),
  ...
  const vpe_section SectionTable[NumberOfSections] = {
      {{'.', 'i', 'd', 'a', 't', 'a', '$', '2'},
       u32(0),
       u32(0),
       u32(sizeof(vpe_import_directory_table_entry)),
       u32(sizeof(vpe_file_header) + NumberOfSections * sizeof(vpe_section)),
       u32(sizeof(vpe_file_header) + NumberOfSections * sizeof(vpe_section) +
           sizeof(vpe_import_directory_table_entry)),
       u32(0),
  ...
  // .idata$2
  const vpe_import_directory_table_entry ImportDescriptor{
      u32(0), u32(0), u32(0), u32(0), u32(0),
  };
  append(Buffer, ImportDescriptor);

  const vpe_relocation RelocationTable[NumberOfRelocations] = {
      {u32(offsetof(vpe_import_directory_table_entry, NameRVA)), u32(2),
       u16(getImgRelRelocation(Machine))},
      {u32(offsetof(vpe_import_directory_table_entry, ImportLookupTableRVA)),
       u32(3), u16(getImgRelRelocation(Machine))},
      {u32(offsetof(vpe_import_directory_table_entry, ImportAddressTableRVA)),
  ...
  // Symbol Table
  vpe_symbol16 SymbolTable[NumberOfSymbols] = {
      {{{0, 0, 0, 0, 0, 0, 0, 0}},
       u32(0),
       u16(1),
       u16(0),
...
ObjectFactory::createNullImportDescriptor(std::vector<uint8_t> &Buffer) {
  ...
  const vpe_section SectionTable[NumberOfSections] = {
      {{'.', 'i', 'd', 'a', 't', 'a', '$', '3'},
       ...
       u32(0),
       u32(sizeof(vpe_import_directory_table_entry)),
       u32(sizeof(vpe_file_header) +
           (NumberOfSections * sizeof(vpe_section))),
       u32(0),
       u32(0),
       u16(0),
       u16(0),
...
NewArchiveMember ObjectFactory::createShortImport(StringRef Sym,
  ...
  size_t Size = sizeof(vpe_import_header) + ImpSize;
  char *Buf = Alloc.Allocate<char>(Size);
  memset(Buf, 0, Size);
  char *P = Buf;

  // Write short import library.
  auto *Imp = reinterpret_cast<vpe_import_header *>(P);
  P += sizeof(*Imp);
  Imp->Sig2 = 0xFFFF;
  Imp->Machine = Machine;
