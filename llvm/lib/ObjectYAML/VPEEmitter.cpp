//===- yaml2vpe - Convert YAML to a VPE object file -----------------------===//
//
...
/// This parses a yaml stream that represents a VPE object file.
/// See docs/yaml2obj for the yaml scheema.
struct VPEParser {
  VPEParser(VPEYAML::Object &Obj, yaml::ErrorHandler EH)
      : Obj(Obj), SectionTableStart(0), SectionTableSize(0), ErrHandler(EH) {
    // A VPE string table always starts with a 4 byte size field. Offsets into
  ...
  bool useBigObj() const {
    return static_cast<int32_t>(Obj.Sections.size()) >
           VPE::MaxNumberOfSections16;
  }

  bool isPE() const { return Obj.OptionalHeader.has_value(); }
  bool is64Bit() const {
    return Obj.Header.Machine == VPE::IMAGE_FILE_MACHINE_AMD64 ||
           Obj.Header.Machine == VPE::IMAGE_FILE_MACHINE_ARM64 ||
           Obj.Header.Machine == VPE::IMAGE_FILE_MACHINE_ARM64EC;
  }
  ...
  unsigned getHeaderSize() const {
    return useBigObj() ? VPE::Header32Size : VPE::Header16Size;
  }

  unsigned getSymbolSize() const {
    return useBigObj() ? VPE::Symbol32Size : VPE::Symbol16Size;
  }

  bool parseSections() {
    for (VPEYAML::Section &Sec : Obj.Sections) {
      // If the name is less than 8 bytes, store it in place, otherwise
      // store it in the string table.
      StringRef Name = Sec.Name;

      if (Name.size() <= VPE::NameSize) {
  ...
  bool parseSymbols() {
    for (VPEYAML::Symbol &Sym : Obj.Symbols) {
      // If the name is less than 8 bytes, store it in place, otherwise
      // store it in the string table.
      ...
      Sym.Header.Type |= Sym.ComplexType << VPE::SCT_COMPLEX_TYPE_SHIFT;
    }
    return true;
  }
...
static bool layoutOptionalHeader(VPEParser &CP) {
  if (!CP.isPE())
    return true;
  unsigned PEHeaderSize = CP.is64Bit() ? sizeof(object::vpe_pe32plus_header)
                                       : sizeof(object::vpe_pe32_header);
  CP.Obj.Header.SizeOfOptionalHeader =
      PEHeaderSize + sizeof(object::vpe_data_directory) *
                         CP.Obj.OptionalHeader->Header.NumberOfRvaAndSizes;
  return true;
}
...
toDebugS(ArrayRef<CodeViewYAML::YAMLDebugSubsection> Subsections,
  ...
  Err(Writer.writeInteger<uint32_t>(VPE::DEBUG_SECTION_MAGIC));
...
// Take a CP and assign addresses and sizes to everything. Returns false if the
// layout is not valid to do.
static bool layoutVPE(VPEParser &CP) {
  // The section table starts immediately after the header, including the
  // optional header.
  CP.SectionTableStart =
      CP.getHeaderSize() + CP.Obj.Header.SizeOfOptionalHeader;
  if (CP.isPE())
    CP.SectionTableStart += DOSStubSize + sizeof(VPE::PEMagic);
  CP.SectionTableSize = VPE::SectionSize * CP.Obj.Sections.size();

  uint32_t CurrentSectionDataOffset =
      CP.SectionTableStart + CP.SectionTableSize;

  for (VPEYAML::Section &S : CP.Obj.Sections) {
  ...
  for (VPEYAML::Section &S : CP.Obj.Sections) {
    ...
    if (S.SectionData.binary_size() > 0) {
      ...
      if (!S.Relocations.empty()) {
        ...
        if (S.Header.Characteristics & VPE::IMAGE_SCN_LNK_NRELOC_OVFL) {
          S.Header.NumberOfRelocations = 0xffff;
          CurrentSectionDataOffset += VPE::RelocationSize;
        } else
          S.Header.NumberOfRelocations = S.Relocations.size();
        CurrentSectionDataOffset += S.Relocations.size() * VPE::RelocationSize;
      }
    } else {
  ...
  for (std::vector<VPEYAML::Symbol>::iterator i = CP.Obj.Symbols.begin(),
...
template <typename T>
static uint32_t initializeOptionalHeader(VPEParser &CP, uint16_t Magic,
                                         T Header) {
  ...
  for (const VPEYAML::Section &S : CP.Obj.Sections) {
    if (S.Header.Characteristics & VPE::IMAGE_SCN_CNT_CODE)
      SizeOfCode += S.Header.SizeOfRawData;
    if (S.Header.Characteristics & VPE::IMAGE_SCN_CNT_INITIALIZED_DATA)
      SizeOfInitializedData += S.Header.SizeOfRawData;
    if (S.Header.Characteristics & VPE::IMAGE_SCN_CNT_UNINITIALIZED_DATA)
  ...
  Header->NumberOfRvaAndSize = CP.Obj.OptionalHeader->Header.NumberOfRvaAndSizes;
  return BaseOfData;
}

static bool writeVPE(VPEParser &CP, raw_ostream &OS) {
  if (CP.isPE()) {
    // Write out the PE signature.
    OS.write(VPE::PEMagic, sizeof(VPE::PEMagic));
  }
  if (CP.useBigObj()) {
    OS << binary_le(static_cast<uint16_t>(VPE::IMAGE_FILE_MACHINE_UNKNOWN))
       << binary_le(static_cast<uint16_t>(0xffff))
       << binary_le(
              static_cast<uint16_t>(VPE::BigObjHeader::MinBigObjectVersion))
       << binary_le(CP.Obj.Header.Machine)
       << binary_le(CP.Obj.Header.TimeDateStamp);
    OS.write(VPE::BigObjMagic, sizeof(VPE::BigObjMagic));
  ...
  if (CP.isPE()) {
    if (CP.is64Bit()) {
      object::vpe_pe32plus_header PEH;
      initializeOptionalHeader(CP, VPE::PE32Header::VPE32_PLUS, &PEH);
      OS.write(reinterpret_cast<char *>(&PEH), sizeof(PEH));
    } else {
      object::vpe_pe32_header PEH;
      uint32_t BaseOfData =
          initializeOptionalHeader(CP, VPE::PE32Header::VPE32, &PEH);
      PEH.BaseOfData = BaseOfData;
      OS.write(reinterpret_cast<char *>(&PEH), sizeof(PEH));
    }
    for (uint32_t I = 0; I < CP.Obj.OptionalHeader->Header.NumberOfRvaAndSizes;
         ++I) {
      const std::optional<VPE::DataDirectory> *DataDirectories =
  ...
  for (const VPEYAML::Section &S : CP.Obj.Sections) {
    OS.write(S.Header.Name, VPE::NameSize);
  ...
  for (const VPEYAML::Symbol &Sym : CP.Obj.Symbols) {
  ...
  for (const VPEYAML::Section &S : CP.Obj.Sections) {
    ...
    for (const VPEYAML::Relocation &R : S.Relocations) {
  ...
  for (std::vector<VPEYAML::Symbol>::const_iterator i = CP.Obj.Symbols.begin(),
                                                     e = CP.Obj.Symbols.end();
       i != e; ++i) {
    OS.write(i->Header.Name, VPE::NameSize);
    ...
    if (i->FunctionDefinition) {
      ...
      OS.write_zeros(CP.getSymbolSize() - VPE::Symbol16Size);
    }
    if (i->bfAndefSymbol) {
...
bool yaml2vpe(llvm::VPEYAML::Object &Doc, raw_ostream &Out,
               ErrorHandler ErrHandler) {
  VPEParser CP(Doc, ErrHandler);
  if (!CP.parse()) {
  ...
  if (!layoutOptionalHeader(CP)) {
    ErrHandler("failed to layout optional header for VPE file");
    return false;
  }

  if (!layoutVPE(CP)) {
    ErrHandler("failed to layout VPE file");
    return false;
  }
  if (!writeVPE(CP, Out)) {
    ErrHandler("failed to write VPE file");
    return false;
  }
  return true;
}
