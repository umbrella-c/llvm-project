//===- COFF.h - VPE object file implementation -----------------*- C++ -*-===//
//
...
using vpe_symbol16 = vpe_symbol<support::ulittle16_t>;
using vpe_symbol32 = vpe_symbol<support::ulittle32_t>;
...
struct vpe_aux_section_definition;
struct vpe_aux_weak_external;

class VPESymbolRef {
public:
  VPESymbolRef() = default;
  VPESymbolRef(const vpe_symbol16 *CS) : CS16(CS) {}
  VPESymbolRef(const vpe_symbol32 *CS) : CS32(CS) {}

  const void *getRawPtr() const {
    return CS16 ? static_cast<const void *>(CS16) : CS32;
  }

  const vpe_symbol_generic *getGeneric() const {
    if (CS16)
      return reinterpret_cast<const vpe_symbol_generic *>(CS16);
    return reinterpret_cast<const vpe_symbol_generic *>(CS32);
  }

  friend bool operator<(VPESymbolRef A, VPESymbolRef B) {
  ...
  bool isBigObj() const {
    ...
    if (CS32)
      return true;
    llvm_unreachable("VPESymbolRef points to nothing!");
  }
...
private:
  ...
  const vpe_symbol16 *CS16 = nullptr;
  const vpe_symbol32 *CS32 = nullptr;
};
...
struct vpe_relocation {
  support::ulittle32_t VirtualAddress;
  support::ulittle32_t SymbolTableIndex;
  support::ulittle16_t Type;
};

struct vpe_aux_function_definition {
...
static_assert(sizeof(vpe_aux_function_definition) == 18,
              "auxiliary entry must be 18 bytes");

struct vpe_aux_bf_and_ef_symbol {
  char Unused1[4];
...
static_assert(sizeof(vpe_aux_bf_and_ef_symbol) == 18,
              "auxiliary entry must be 18 bytes");
...
static_assert(sizeof(vpe_aux_weak_external) == 18,
              "auxiliary entry must be 18 bytes");
...
static_assert(sizeof(vpe_aux_section_definition) == 18,
              "auxiliary entry must be 18 bytes");

struct vpe_aux_clr_token {
  uint8_t              AuxType;
  uint8_t              Reserved;
  support::ulittle32_t SymbolTableIndex;
  char                 MBZ[12];
...
static_assert(sizeof(vpe_aux_clr_token) == 18,
              "auxiliary entry must be 18 bytes");

struct vpe_import_header {
  support::ulittle16_t Sig1;
  support::ulittle16_t Sig2;
  support::ulittle16_t Version;
  support::ulittle16_t Machine;
...
struct vpe_import_directory_table_entry {
...
using vpe_tls_directory32 = vpe_tls_directory<support::little32_t>;
using vpe_tls_directory64 = vpe_tls_directory<support::little64_t>;

/// Bits in control flow guard flags as we understand them.
enum class vpe_guard_flags : uint32_t {
...
struct vpe_load_configuration32 {
  support::ulittle32_t Size;
  support::ulittle32_t TimeDateStamp;
  support::ulittle16_t MajorVersion;
  support::ulittle16_t MinorVersion;
  ...
  support::ulittle32_t GuardFlags; // vpe_guard_flags

  // Added in MSVC 2017
  vpe_load_config_code_integrity CodeIntegrity;
...
struct vpe_load_configuration64 {
  support::ulittle32_t Size;
  support::ulittle32_t TimeDateStamp;
  support::ulittle16_t MajorVersion;
  support::ulittle16_t MinorVersion;
...
struct vpe_runtime_function_x64 {
  support::ulittle32_t BeginAddress;
  support::ulittle32_t EndAddress;
  support::ulittle32_t UnwindInformation;
};

struct vpe_base_reloc_block_header {
  support::ulittle32_t PageRVA;
  support::ulittle32_t BlockSize;
};

struct vpe_base_reloc_block_entry {
...
struct vpe_resource_data_entry {
...
private:
  ...
  const vpe_tls_directory32 *TLSDirectory32;
  const vpe_tls_directory64 *TLSDirectory64;
  ...
  const vpe_symbol_type *toSymb(DataRefImpl Symb) const;
  const vpe_section *toSec(DataRefImpl Sec) const;
  const vpe_relocation *toRel(DataRefImpl Rel) const;
...
public:
  ...
  uint16_t getMachine() const {
    if (VPEHeader)
      return VPEHeader->Machine;
    if (VPEBigObjHeader)
      return VPEBigObjHeader->Machine;
    llvm_unreachable("no VPE header!");
  }

  uint16_t getSizeOfOptionalHeader() const {
    if (VPEHeader)
      return VPEHeader->isImportLibrary() ? 0
                                           : VPEHeader->SizeOfOptionalHeader;
  ...
  uint16_t getCharacteristics() const {
    if (VPEHeader)
      return VPEHeader->isImportLibrary() ? 0 : VPEHeader->Characteristics;
  ...
  uint32_t getTimeDateStamp() const {
    if (VPEHeader)
      return VPEHeader->TimeDateStamp;
    if (VPEBigObjHeader)
      return VPEBigObjHeader->TimeDateStamp;
    llvm_unreachable("no VPE header!");
  }

  uint32_t getNumberOfSections() const {
    if (VPEHeader)
      return VPEHeader->isImportLibrary() ? 0 : VPEHeader->NumberOfSections;
    if (VPEBigObjHeader)
      return VPEBigObjHeader->NumberOfSections;
    llvm_unreachable("no VPE header!");
  }
  ...
  uint32_t getPointerToSymbolTable() const {
    if (VPEHeader)
      return VPEHeader->isImportLibrary() ? 0
                                           : VPEHeader->PointerToSymbolTable;
    if (VPEBigObjHeader)
      return VPEBigObjHeader->PointerToSymbolTable;
    llvm_unreachable("no VPE header!");
  }

  uint32_t getRawNumberOfSymbols() const {
    if (VPEHeader)
      return VPEHeader->isImportLibrary() ? 0 : VPEHeader->NumberOfSymbols;
    if (VPEBigObjHeader)
      return VPEBigObjHeader->NumberOfSymbols;
    llvm_unreachable("no VPE header!");
  }
  ...
  const vpe_load_configuration32 *getLoadConfig32() const {
    assert(!is64());
    return reinterpret_cast<const vpe_load_configuration32 *>(LoadConfig);
  }

  const vpe_load_configuration64 *getLoadConfig64() const {
    assert(is64());
    return reinterpret_cast<const vpe_load_configuration64 *>(LoadConfig);
  }
...
public:
  VPEObjectFile(MemoryBufferRef Object);
  ...
  const vpe_section *getVPESection(const SectionRef &Section) const;
  VPESymbolRef getVPESymbol(const DataRefImpl &Ref) const;
  VPESymbolRef getVPESymbol(const SymbolRef &Symbol) const;
  const vpe_relocation *getVPERelocation(const RelocationRef &Reloc) const;
  ...
  const vpe_tls_directory32 *getTLSDirectory32() const {
    return TLSDirectory32;
  }
  const vpe_tls_directory64 *getTLSDirectory64() const {
  ...
  const vpe_file_header *getCOFFHeader() const { return VPEHeader; }
  const vpe_bigobj_file_header *getCOFFBigObjHeader() const {
    return VPEBigObjHeader;
  }

  const vpe_pe32_header *getPE32Header() const { return PE32Header; }
  const vpe_pe32plus_header *getPE32PlusHeader() const { return PE32PlusHeader; }
 
  const vpe_data_directory *getDataDirectory(uint32_t index) const;
  Expected<const vpe_section *> getSection(int32_t index) const;

  Expected<VPESymbolRef> getSymbol(uint32_t index) const {
    if (index >= getNumberOfSymbols())
      return errorCodeToError(object_error::parse_failed);
    if (SymbolTable16)
      return VPESymbolRef(SymbolTable16 + index);
    if (SymbolTable32)
      return VPESymbolRef(SymbolTable32 + index);
    return errorCodeToError(object_error::parse_failed);
  }

  template <typename T>
  Error getAuxSymbol(uint32_t index, const T *&Res) const {
    Expected<VPESymbolRef> S = getSymbol(index);
  ...
  Expected<StringRef> getSymbolName(VPESymbolRef Symbol) const;
  Expected<StringRef> getSymbolName(const vpe_symbol_generic *Symbol) const;

  ArrayRef<uint8_t> getSymbolAuxData(VPESymbolRef Symbol) const;

  uint32_t getSymbolIndex(VPESymbolRef Symbol) const;

  size_t getSymbolTableEntrySize() const {
    if (VPEHeader)
      return sizeof(vpe_symbol16);
    if (VPEBigObjHeader)
      return sizeof(vpe_symbol32);
    llvm_unreachable("null symbol table pointer!");
  }

  ArrayRef<vpe_relocation> getRelocations(const vpe_section *Sec) const;

  Expected<StringRef> getSectionName(const vpe_section *Sec) const;
  uint64_t getSectionSize(const vpe_section *Sec) const;
  Error getSectionContents(const vpe_section *Sec,
  ...
  Error getDebugPDBInfo(const vpe_debug_directory *DebugDir,
...
public:
  ...
  Error
  getImportTableEntry(const vpe_import_directory_table_entry *&Result) const;

private:
  const vpe_import_directory_table_entry *ImportTable;
  uint32_t Index;
...
private:
  const vpe_base_reloc_block_header *Header;
  uint32_t Index;
};
...
public:
  ...
  getEntrySubDir(const vpe_resource_dir_entry &Entry);
  Expected<const vpe_resource_dir_table &> getBaseTable();

private:
  BinaryByteStream BBS;

  Expected<const vpe_resource_dir_table &> getTableAtOffset(uint32_t Offset);
