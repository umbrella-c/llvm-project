...
public:
  ...
  using symbol_list = DenseSet<VPESymbol *>;
...
VPESymbol *VPEObjectWriter::createSymbol(StringRef Name) {
  Symbols.push_back(std::make_unique<VPESymbol>(Name));
...
VPESection *VPEObjectWriter::createSection(StringRef Name) {
  Sections.emplace_back(std::make_unique<VPESection>(Name));
...
void VPEObjectWriter::writeSectionHeaders() {
  ...
  llvm::sort(Arr, [](const VPESection *A, const VPESection *B) {
...
void VPEObjectWriter::setWeakDefaultNames() {
...
uint64_t VPEObjectWriter::writeObject(MCAssembler &Asm,
  ...
  for (auto &Section : Sections) {
    ...
    const auto *AssocMCSec = cast<MCSectionVPE>(&AssocMCSym->getSection());
    assert(SectionMap.count(AssocMCSec));
    VPESection *AssocSec = SectionMap[AssocMCSec];
...
std::unique_ptr<MCObjectWriter> llvm::createVPEObjectWriter(
    std::unique_ptr<MCVPEObjectTargetWriter> MOTW, raw_pwrite_stream &OS) {
  return std::make_unique<VPEObjectWriter>(std::move(MOTW), OS);
}
